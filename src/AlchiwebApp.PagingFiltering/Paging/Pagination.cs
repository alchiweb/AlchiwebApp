...
public class Pagination
{
    ...
    public Pagination(int itemsCount, IBasePaging baseFilter)
        : this(new PaginationSettings(10, 100), itemsCount, baseFilter.PageSize, baseFilter.Page)
    {
    }

    public Pagination(PaginationSettings paginationSettings, int itemsCount, IBasePaging baseFilter)
    ...
    public Pagination(PaginationSettings paginationSettings, int itemsCount, int? pageSize, int? page)
    {
        ...
        StartItem = PageSize == 0 ? 0 : (TotalItems == 0 ? 0 : PageSize * (Page - 1) + 1);
    ...
    private int GetHandledPage(int? page)
    {
        ...
        if (!page.HasValue || page <= 0)
            return _paginationSettings.DefaultPage;
