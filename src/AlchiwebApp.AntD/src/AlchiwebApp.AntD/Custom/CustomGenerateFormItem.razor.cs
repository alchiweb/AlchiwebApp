...
/*
public partial class CustomGenerateFormItem<TModel> : ComponentBase
{
    private PropertyInfo[] _propertyInfos = { };

    [CascadingParameter(Name = "Form")] private IForm Form { get; set; } = null!;
    ...
    [Parameter] public bool WithPlaceholder { get; set; } = false;
        ...
        RenderTreeBuilder builder, int sequenceIndex)
    {
        ...
        // not needed: FormItem construct it him-self...
        //builder.AddAttribute(sequenceIndex++, "Label", GetDisplayName(property) ?? "");
        if (!WithLabel)
        {
            builder.AddAttribute(sequenceIndex++, "Label", "");
        }
    ...
    private void GenerateByType(PropertyInfo property, RenderTreeBuilder builder, object model, string structurePath)
    {
        ...
        if (underlyingType.IsNumericType() && !underlyingType.IsEnum)
        {
    ...
    private RenderFragment MakeFormItem(PropertyInfo property, Type formItemChildContent, object model)
    {
        ...
        {
            int sequenceIndex = 0;
            builder.OpenComponent(sequenceIndex++, constructedType);
            if (WithPlaceholder)
            {
                builder.AddAttribute(sequenceIndex++, "Placeholder", GetDisplayName(property) ?? "");
            }

            builder.AddAttribute(sequenceIndex++, "Value", property.GetValue(model));
            var eventCallback = CreateEventCallback(property.PropertyType, this,
                new Action<object>(o => property.SetValue(model, o)));
            builder.AddAttribute(sequenceIndex++, "ValueChanged", eventCallback);
            builder.AddAttribute(sequenceIndex++, "ValueExpression", Expression.Lambda(constructedFuncType, exp));
            if (isReadOnly)
            {
                builder.AddAttribute(sequenceIndex++, "Disabled", isReadOnly);
            }
    ...
    {
        if (property?.GetCustomAttribute<DisplayNameAttribute>(true) is DisplayNameAttribute displayNameAttribute && !string.IsNullOrEmpty(displayNameAttribute.DisplayName))
        {
            return displayNameAttribute.DisplayName;
        }

        return property?.GetCustomAttribute<DisplayAttribute>(true) is DisplayAttribute displayAttribute
