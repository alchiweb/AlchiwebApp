...
/// </summary>
public sealed class RenderImage : IBlockRenderer<ImageData>
{
    /// <summary>
    ...
    public static void Render(EditorJsRenderTreeBuilder render_tree_builder, EditorJsBlock<ImageData> block)
    {
        string? id = block.Id;
        ...
        EditorJsStylingMap? css = render_tree_builder.StylingMap.FirstOrDefault(item => item.Type == BlockTypeEnum.Image && item.Id == id);
        css ??= render_tree_builder.StylingMap.FirstOrDefault(item => item.Type == BlockTypeEnum.Image && item.Id == null);
